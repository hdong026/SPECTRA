...
class BSplineKANLinear(nn.Module):
    def __init__(self, in_features, out_features, grid_size=5, spline_order=3, scale_noise=0.1, scale_base=1.0, scale_spline=1.0):
        super(BSplineKANLinear, self).__init__()
    ...
    def b_splines(self, x: torch.Tensor):
        ...
        return bases # [..., in, grid_size + order]
    ...
    def forward(self, x):
        ...
        spline_output = torch.einsum("...ic,oic->...o", bases, self.spline_weight)
        
        return self.scale_base * base_output + self.scale_spline * spline_output


class KASA_v2_w_bspline(nn.Module):
    def __init__(self, **model_args):
        super(KASA_v2_w_bspline, self).__init__()
        ...
        if self.input_dim > 3:
            ...
            print(">>> [KASA Ablation] Experiment 4: RBF-KAN replaced by Standard B-Spline KAN <<<")
        else:
             print(">>> [WARNING] Input Dim is 3! This is Experiment 1 setting, not Experiment 4! Check Config! <<<")
